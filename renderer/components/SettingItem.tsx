...
import {Card, CardHeader, CardBody, CardFooter, RadioGroup, Radio, Slider, Autocomplete, AutocompleteItem} from "@nextui-org/react"
...
const SettingItem = (props) => {
  ...
  useEffect(() => {
    if (item.name === 'signaling_cloud' || item.name === 'signaling_home') {
      const method = item.name === 'signaling_cloud' ? 'getXcloudToken' : 'getXhomeToken'
  ...
  const handleChangeSetting = value => {
    ...
    if (key) {
      if (item.name === 'signaling_cloud' || item.name === 'signaling_home') {
        const method = item.name === 'signaling_cloud' ? 'setXcloudTokenDefault' : 'setXhomeTokenDefault'
